...
另外还提供 appCheck 接口给 Uptime Kuma 等其他监控使用，来检查业务是否异常
...
为了便于分析问题，本地增加日志文件记录，路径为 `/storage/emulated/0/Android/data/com.vone.vmq/files/logs`，保留最近 7 天的日志

同时也在界面增加日志显示以及一键清理、修复部分 bug、移除手工配置（使用扫码配置就可以），以及其他一些优化
...
如果 `设备和应用通知` 这里被关闭了，也可以在 `应用设置 => 应用管理 => 选择V免签客户端 => 通知管理 => 允许通知` 中进行设置
...
点击开启后，弹出一个危险的窗口，因为应用需要获取展示在通知栏中的消息，通知栏中里面可能会有用户的隐私数据，因此会被系统标记为高危动作

应用仅会读取微信的和支付宝的支付消息，并解析对应的收款金额，不会读取其他消息，也不会存储用户的任何额外数据，请放心使用
...
在 `设置 => 应用设置 => 授权管理 => 自启动管理` 中， 将支付客户端设置为自启动

![](/archives/vmq-deploy/jzhn06.png)

### 关闭省电

将应用设置为电量无限制模式，并且关闭电池中的智能场景省电，在 `设置` 中的 `电池与省电` 下还有很多的配置，能关闭的尽量关闭，省电会让系统杀进程

![](/archives/vmq-deploy/pu5wmp.png)

### 扫码配置

前面的配置完毕之后，打开并通过管理员登录支付的 web 页面，然后在后台页面中切换到 `监控端设置` 菜单，显示如下的配置

![](/archives/vmq-deploy/22q85l.png)

然后在手机应用启动后，点击扫码配置，然后扫描电脑 web 页面上显示的二维码，完成绑定（如果是 https 的，需要先勾选 https 再进行扫码）
